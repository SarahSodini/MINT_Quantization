...
class MyCIFAR10DVS(Subset):
    def __init__(self, root, train_ratio=0.9, data_type="frame", frames_number=10, split_by="number", random_split=False, size=(48, 48)):
        #Converts from numpy event frames to tensor & resize 
        transform_dvs = transforms.Compose([
                lambda x: torch.from_numpy(x), 
                Resize(size=size)])

        dataset_dvs = CIFAR10DVS(
            root=root,
            data_type=data_type, 
            frames_number=frames_number, 
            split_by=split_by, 
            transform=transform_dvs
        )
    ...
    #Split to train test while accounting for class balance
    @staticmethod
    def split_to_train_test_set(train_ratio: float, origin_dataset: torch.utils.data.Dataset, num_classes: int, random_split: bool = False):
        ...
        #populates label_idx. Each class has its own list of sample indices
        ...
        #shuffles each class independently if enabled
        if random_split:
            for i in range(num_classes):
                np.random.shuffle(label_idx[i])

        #splits per class and assigns to train/test
        for i in range(num_classes):
